...
add_action('init', 'create_news_post'); // Использовать функцию только внутри хука init
...
add_action('add_meta_boxes', 'my_news_extra_fields', 1);

function my_news_extra_fields()
{
    add_meta_box('extra_fields', 'Дополнительные поля', 'extra_news_fields_box_func', 'news', 'normal', 'high');
}

// код блока
function extra_news_fields_box_func($post)
{
    ?>
    <p style="display: none"><b>Заголовок новости (он же заголовок страницы) :</b>
...
add_action('save_post', 'my_extra_news_fields_update', 0);

/* Сохраняем данные, при сохранении поста */
function my_extra_news_fields_update($post_id)
{
    // базовая проверка
    if (
        empty($_POST['extra'])
...
function go_news_filter($post_query)
{ // наша функция
...
function true_news_filter_function()
{
...
add_action('wp_ajax_mynewsfilter', 'true_news_filter_function');
add_action('wp_ajax_nopriv_mynewsfilter', 'true_news_filter_function');